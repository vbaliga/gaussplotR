...
ggplot2::ggplot(samp_dat, aes(X_values, Y_values, z = response)) +
...
## The `method` and `orientation_strategy` arguments
...
An additional argument, `orientation_strategy` gives additional control over the
orientation of the fitted Gaussian.  By default, the `orientation_strategy` is
`"unconstrained"`, meaning that the best-fit orientation is returned. 

### `orientation_strategy`
Setting orientation_strategy to a numeric (e.g. `orientation_strategy = pi/2`)
...
Note that supplying a numeric to `orientation_strategy ` is handled differently 
...
`method = "elliptical", orientation_strategy = pi/2` will return parameters for
...
Setting `method = "elliptical_log", orientation_strategy = 0` will result 
in a diagonally-oriented Gaussian, whereas setting `orientation_strategy = -1`
...
As noted above, the `orientation_strategy` can be used to dictate the 
...
Again, setting the value of `Q` via `orientation_strategy` will very likely 
